...
// ⚡ Bolt: Cache system timestamp to avoid redundant Date.now() calls and use Math.round for precision rounding to avoid heavy string allocation and GC overhead of .toFixed()
const mock = () => {
  const now = Date.now();
  return {
    type: 'mock',
