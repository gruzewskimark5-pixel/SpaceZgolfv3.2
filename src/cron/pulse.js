...
const fetchAPI = async () => { try { const r = await fetch('/api/leaderboard'); if (!r.ok) throw new Error(); const d = await r.json(); return d.map(row => ({ source_module: row.module === 'SPACEZGOLF' ? 'golf_engine' : 'blue_horizon_re', efficiency_coefficient: row.efficiency, domain_kpis: { zscore: row.zscore }, signal_status: row.signal, system_timestamp: row.timestamp || Date.now() })); } catch { return null; } };
export const startPulse = (ms = 60000) => {
  if (intervalId) clearInterval(intervalId);
  const tick = async () => { const data = await fetchAPI() || mock(); const lb = zScoreBoard(data); StateStore.set({ leaderboard: lb, lastPulse: Date.now() }); EventBus.emit('leaderboard:updated', lb); };
