...
## 2024-06-05 - Avoid String Conversions for Math Rounding
**Learning:** In high-iteration loops where mathematical precision needs to be rounded, using `Number(val.toFixed(precision))` forces the numeric value to be converted into a string, then back into a number, which creates heavy garbage collection overhead. Furthermore, casting values redundantly when calling a helper function adds measurable parser overhead.
**Action:** When calculating and rounding mathematical values, use integer math (e.g., `Math.round(val * 10000) / 10000`) rather than `.toFixed()`. Ensure values are only converted into numbers once, and not redundantly inside helper functions.
