...
## 2026-05-21 - Precompute ETag to bypass Express hashing overhead\n**Learning:** Express.js's default `res.send()` synchronously computes an MD5 hash to generate ETags for string payloads on every request, which adds CPU overhead even when serving static cached data. However, if the `ETag` header is already set, Express correctly bypasses this hashing.\n**Action:** When serving string-serialized caches (like the leaderboard JSON cache), precompute the ETag when generating the cache and explicitly call `res.setHeader('ETag', etag)` before `res.send()` to eliminate the repeated MD5 hashing overhead on cache hits.
