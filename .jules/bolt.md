...
## 2024-06-09 - Client-Side ETag Polling Optimization\n**Learning:** When polling a backend API on a rapid interval, constantly fetching, parsing, and re-rendering identical data wastes significant client CPU and memory. Backend ETag caching alone only saves backend cycles; the frontend still processes the 200 OK response.\n**Action:** When writing polling mechanisms, implement HTTP conditional requests by capturing the `ETag` header from successful responses and sending it back via `If-None-Match`. Short-circuit the entire processing pipeline early on a `304 Not Modified` response to eliminate unnecessary JSON parsing, data transformations, and DOM layout thrashing during idle periods.
