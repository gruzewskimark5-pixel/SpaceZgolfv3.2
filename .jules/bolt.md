...
## 2024-06-25 - Express.js Static Middleware Positioning
**Learning:** Placing `express.static` before API routes in an Express.js application forces a filesystem `stat` call on every API request.
**Action:** Optimize by placing `express.static` below API routes, just above the catch-all/fallback route handlers, to prevent this unnecessary I/O overhead on high-throughput endpoints.
