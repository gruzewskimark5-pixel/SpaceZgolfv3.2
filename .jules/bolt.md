...
## 2026-08-26 - [Express.js Middleware Ordering Optimization]
**Learning:** Placing `express.static` before API routes in an Express.js application forces a filesystem `stat` call on every incoming request, even if it's meant for an API route. This introduces unnecessary I/O overhead on high-throughput endpoints.
**Action:** Always place `express.static` below API routes, just above the catch-all/fallback route handlers, to prevent this overhead.
