...
## 2024-05-20 - Express ETag Generation Overhead
**Learning:** Express.js's default res.send() synchronously computes an MD5 hash to generate ETags for string payloads.
**Action:** Precompute the ETag when caching and set it via res.setHeader('ETag', precomputedETag) before calling res.send() to skip internal hashing.
