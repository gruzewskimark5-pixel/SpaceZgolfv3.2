...
## 2024-08-25 - express.static middleware placement
**Learning:** Placing `express.static` before API routes in an Express.js application forces a filesystem `stat` call on every API request. This causes unnecessary I/O overhead on high-throughput endpoints.
**Action:** Optimize by placing `express.static` below API routes, just above the catch-all/fallback route handlers, to prevent this unnecessary I/O overhead.
