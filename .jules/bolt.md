...
## 2024-06-05 - Inlining Calculations in Hot Loops
**Learning:** Extracting small calculations into helper functions (like `calcDI` and `normalizeZ`) or using closure functions (like `mapRow`) inside a loop introduces V8 function call overhead and closure allocation costs. When iterating over thousands of items (e.g., in `memStore` or database results), inlining the mathematical calculations directly inside the loop reduces execution time significantly.
**Action:** In performance-critical array iterations, inline small helper functions and closures into the loop body to avoid function call overhead and improve execution speed, particularly when calculating metrics like the dominance index.
