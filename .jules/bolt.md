...
## 2026-08-31 - Optimize Express Static Middleware Placement
**Learning:** Placing `express.static` before API routes forces a filesystem `stat` call on every API request, adding unnecessary I/O overhead.
**Action:** Always place `express.static` below API routes and just above catch-all/fallback route handlers to maximize API throughput.
