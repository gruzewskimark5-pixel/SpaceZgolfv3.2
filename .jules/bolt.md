...
## 2026-08-29 - [Express.js static middleware I/O Overhead]
**Learning:** Placing `express.static` before API routes forces a filesystem `stat` call on every API request.
**Action:** Always place `express.static` below API routes, just above catch-all routes, to avoid unnecessary I/O overhead on high-throughput endpoints.
