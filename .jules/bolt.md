...
## 2026-09-01 - Express.js static middleware positioning\n**Learning:** Placing `express.static` before API routes forces a filesystem `stat` call on every API request. This adds unnecessary I/O overhead on high-throughput API endpoints.\n**Action:** Optimize by placing `express.static` below API routes, just above the catch-all/fallback route handlers, to prevent this unnecessary I/O overhead.
