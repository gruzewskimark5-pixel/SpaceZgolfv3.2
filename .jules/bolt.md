...
## 2024-06-05 - Redundant Cache Overhead
**Learning:** Sometimes the codebase might contain conflicting or duplicate cache implementations resulting from previous refactors. In the `/api/leaderboard` endpoint, there were two overlapping object caching techniques; one returned a raw object forcing an overhead `res.json(result)` and the other returned a pre-serialized string saving `JSON.stringify` overhead.
**Action:** When inspecting caching logic, remove inferior/redundant caches (like caching an object just to `res.json()` it) and keep pre-serialized caches that avoid continuous stringification overhead on reads.
