...
app.get('/api/leaderboard', async (req, res) => {
  try {
    ...
    if (leaderboardCache) {
      ...
      // ⚡ Bolt: Avoid synchronous MD5 generation in Express res.send() by providing a pre-computed ETag
      if (leaderboardETag) res.setHeader('ETag', leaderboardETag);
    ...
    // ⚡ Bolt: Pre-calculate the ETag hash here so Express doesn't have to re-hash the string on every single GET
