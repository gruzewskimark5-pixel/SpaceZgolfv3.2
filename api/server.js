...
app.post('/api/global/vitals', async (req, res) => {
  try {
    ...
    if (supabase) {
      ...
      if (error) {
        for (let i = 0; i < len; i++) memStore.set(rows[i].source_module, rows[i]);
      }
    } else {
      // ⚡ Bolt: Replace forEach with a standard for loop. This provides a minor
      // speedup by avoiding callback allocation overhead when populating the memStore map.
      for (let i = 0; i < len; i++) memStore.set(rows[i].source_module, rows[i]);
    }
  ...
  try {
    ...
    if (supabase) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        // This provides measurable speedups in large V8 hot loops.
